...
public class StatikApp
{
    ...
    private final double[][] biegemoment_werte;
    ...
    private static int getCase(double position_va, double position_ha,
            double x, double laenge)
    {
        ...
        if (position_ha > 0 && equalsLess(x, position_va)
                && equalsLess(x, position_ha))
        {
            return 3;
        }
        ...
        if (position_ha > 0 && x > position_ha && x > position_va
                && equalsLess(position_va, laenge))
        {
            return 5;
        }
        if (position_ha > 0 && equalsLess(x, position_ha)
                && position_va > laenge)
        {
            return 6;
        }
    ...
    public StatikApp() throws Exception
    {
        ...
        do
        {
            System.out.print("Geben Sie das Gesamtgewicht des "
                    + "Zweiachsers ein [5 - 10 t]: ");
        ...
        do
        {
            System.out.print("Geben Sie den Achsabstand des "
                    + "Zweiachsers ein [4 - 10 m]: ");
            if (!scan.hasNextDouble())
            {
                scan.next();
                continue;
        ...
        do
        {
            System.out.print("Geben Sie die Breite des "
                    + "Fahrzeugs ein [1 - 5 m]: ");
            if (!scan.hasNextDouble())
            {
                scan.next();
        ...
        do
        {
            System.out.print("Geben Sie die Querschnittsmaße des "
                    + "Trägers ein [b h s t (in cm)]: ");
        ...
        while (b_in <= 0 || h_in <= 0 || s_in <= 0
                || t_in <= 0 || b_in <= s_in || h_in <= 2 * t_in);

        //fill objects
        lkw = new LKW(hoehe_in, breite_in, gesamt_gewicht_in * 1000,
                firma_in, fahrer_in, achsen_abstand_in);
    ...
    public void berechne()
    {
        ...
        double gewichtskraft_traeger = traegerDichte * erdBeschleunigung
                * brueckenlaenge * 10 * traeger.getQuerschnitt().getFlaeche()
                / 10000.0;
        ;
        // N/m
        double streckenlast_traeger_gesamt = (eigenLast + gewichtskraft_traeger
                / brueckenlaenge) * sicherheitEigenlast;

        double p_z_va = lkw.achsLastVA() * erdBeschleunigung
                * sicherheitVerkehrslast / (double) anzahlTraeger;
        double p_z_ha = lkw.achsLastHA() * erdBeschleunigung
                * sicherheitVerkehrslast / (double) anzahlTraeger;
        double m_d = 0;
        double m_z_ha = 0;
        double m_z_va = 0;
        ...
        for (position_va = 0; position_va < max_position;
                position_va += 0.1, m_z_ha = 0, m_z_va = 0, m_d = 0, biegeSpannung = 0)
        {
            ...
            for (x = 0; x < brueckenlaenge; x += 0.1)
            {
                // Statisches Moment!!!
                // Nm
                m_d = (x * (brueckenlaenge - x)) / 2.0
                        * streckenlast_traeger_gesamt;

                auswahl = StatikApp.getCase(position_va, position_ha, x,
                        brueckenlaenge);
                ...
                switch (auswahl)
                {
                    case 1:
                    {
                        m_z_va = p_z_va * (x / brueckenlaenge) * (brueckenlaenge
                                - position_va);
                        m_z_ha = 0;
                        break;
                    }
                    case 2:
                    {
                        m_z_va = p_z_va * ((brueckenlaenge - x) / brueckenlaenge)
                                * position_va;
                        m_z_ha = 0;
                        break;
                    }
                    case 3:
                    {
                        m_z_va = p_z_va * (x / brueckenlaenge) * (brueckenlaenge
                                - position_va);
                        m_z_ha = p_z_ha * (x / brueckenlaenge) * (brueckenlaenge
                                - position_ha);
                        break;
                    }
                    case 4:
                    {
                        m_z_va = p_z_va * (x / brueckenlaenge) * (brueckenlaenge
                                - position_va);
                        m_z_ha = p_z_ha * ((brueckenlaenge - x) / brueckenlaenge)
                                * position_ha;
                        break;
                    }
                    case 5:
                    ...
                    {
                        m_z_va = 0;
                        m_z_ha = p_z_ha * (x / brueckenlaenge)
                                * (brueckenlaenge - position_ha);
                        break;
                    }
                    case 7:
                    {
                ...
                biegemoment_werte[(int) (position_va * 10)][(int) (x * 10)]
                        = belastung;
        ...
        plotter.addDataSet(biegemoment_werte[(int) 
                (position_max_biegemoment * 10)], "Maximale Biegemomente");
    }
            ...
            throws Exception
    {
        ...
        System.out.println(String.format("* Gesamtgewicht: %.2f kg",
                lkw.getGesamtGewicht()));
        System.out.println(String.format("* Achsenabstand: %.2f m",
                lkw.getAchsenAbstand()));
        System.out.println();

        System.out.println("Traeger");
        System.out.println(String.format("* Laenge: %.2f m",
                traeger.getLaenge()));
        System.out.println(String.format("* Gewicht: %.2f kg",
                traeger.getGewicht()));
        System.out.println("* Querschnitt");
        System.out.println(String.format("  * Hoehe: %.2f mm",
                traeger.getQuerschnitt().getHoehe()));
        System.out.println(String.format("  * Breite: %.2f mm",
                traeger.getQuerschnitt().getHoehe()));
        System.out.println(String.format("  * Stegbreite: %.2f mm",
                traeger.getQuerschnitt().getStegBreite()));
        System.out.println(String.format("  * Steghoehe: %.2f mm",
                traeger.getQuerschnitt().getStegBreite()));

        System.out.println("Berechnungskonstanten");
        System.out.println("=====================\n");

        System.out.println(String.format("Erdbeschleunigung: %.2f m/s^2",
                erdBeschleunigung));
        System.out.println(String.format("Traegerdichte: %.2f kg/dm^3",
                traegerDichte));
        System.out.println(String.format("Sicherheit Eigenlast: %.2f",
                sicherheitEigenlast));
        System.out.println(String.format("Sicherheit Verkehrslast: %.2f",
                sicherheitVerkehrslast));
        ...
        System.out.println(String.format("Gewichtskraft Hinterachse: %.2f N",
                lkw.achsLastHA() * erdBeschleunigung));
        System.out.println(String.format("Gewichtskraft Vorderachse: %.2f N",
                lkw.achsLastVA() * erdBeschleunigung));
        System.out.println(String.format("Querschnittsflaeche: %.2f mm^2",
                traeger.getQuerschnitt().getFlaeche()));
        System.out.println(String.format("Flächentraegheitsmoment: %.2f mm^4",
                traeger.getQuerschnitt().getIy()));
        System.out.println(String.format("Maximale Biegespannung: %.2f Nm",
                maxBiegemoment));
        System.out.println(String.format("Vorderachsenposition bei dem "
                + "auftretenden Biegemoment: %.2f m", position_max_biegemoment));
